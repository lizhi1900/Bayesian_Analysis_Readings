...
model{
for(j in 1:17){
       alpha[j]~dnorm(mu+beta*S[j],sigma_a^-2)
       W[j]~dbern(omega*S[j])
  for(i in 1:30){
       z[i,j]~dbern(lambda*S[j]*W[j])
       y[i,j]~    dnorm(alpha[j]+tau*z[i,j],1/((1-z[i,j])*sigma_y^2+z[i,j]*sigma_y2^2))
       y_rep[i,j]~dnorm(alpha[j]+tau*z[i,j],1/((1-z[i,j])*sigma_y^2+z[i,j]*sigma_y2^2))
     }
}
  lambda~dunif(0,1)
  beta~dnorm(0,1)
  ...
  mu~dnorm(0,1)
  sigma_a ~  dt(0,10^-2,1)T(0,)
  sigma_y ~  dt(0,10^-2,1)T(0,)
  sigma_y2~  dt(0,10^-2,1)T(0,)
}'
...
##parameters to save
jags_pars <- c("lambda","beta","tau","omega","y_rep") #
...
par_dat <- posterior_sample %>% spread_draws(lambda,beta,tau,omega)#tidybayes
...
plot(T_min,T_max,type="p",main="Figure 22.3 BDA3",pch = 20,cex=1,xlim=c(.1,.4),ylim=c(.25,.6))
